...
const Fragrances = () => {
  ...
  const [fragranceItem, setFragranceItem] = useState([]);
  ...
  const setGroceryItemMemoized = useCallback(setFragranceItem, []);

  useEffect(() => {
    if (data && data.products) {
      const fragrances = data.products.filter(
        (product) => product.category === "fragrances"
      );
      setFragranceItem(fragrances);
    }
  ...
  return (
    <div className="flex justify-center">
      <div className="w-[94vw] md:h-[510px] h-fit p-4 bg-slate-200 mt-10 mb-3">
        <h1 className="font-semibold text-2xl ml-3"> Buy Your Fragrances </h1>
        <div className="flex justify-center">
          <div className="md:grid md:grid-cols-5 flex flex-col gap-2 mt-4">
            {fragranceItem.map((item) => (
              <div
                key={item.id}
