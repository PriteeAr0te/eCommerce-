...
const HomeDecor = () => {
  ...
  const [homedecorItem, setHomedecorItem] = useState([]);
  ...
  const setGroceryItemMemoized = useCallback(setHomedecorItem, []);

  useEffect(() => {
    if (data && data.products) {
      const homedecors = data.products.filter(
        (product) => product.category === "home-decoration"
      );
      setHomedecorItem(homedecors);
    }
